...
var triangles = 500;
var segments = 500;
var triangleScale = 15;
var segmentScale = 20;


var currentEffect = "";

export const effectVisualizer = (effect, domId) => {
	// console.log(effect, domId)
	currentEffect = effect;
	addEvents();
	init(effect, domId);
	animate(effect, domId);
	...
	function init(effect, domId) {

		container = document.getElementById(domId);
		camera = new THREE.PerspectiveCamera(27, window.innerWidth / window.innerHeight, 20, 20000);
		camera.position.z = 5250;
		camera.position.y = -80;
		camera.position.x = 0;

		scene = new THREE.Scene();
		scene.background = new THREE.Color(0x111827); //OLD 000022
		scene.fog = new THREE.Fog(0x111827, 4000, 5200); //OLD 0x092044

		scene.add(new THREE.AmbientLight(0xffFFFF, 5.1));
	...
	function updatePoints(thisTime) {
		...
		var positions = new Float32Array(triangles * 5 * 3);
		var normals = new Float32Array(triangles * 5 * 5);
		var colors = new Float32Array(triangles * 5 * 5);

		var color = new THREE.Color();

		var n = 700, n2 = n / 2;	// triangles spread in the cube
		var d = 800, d2 = d / 2;	// individual triangle size
...
*/
	...
	function render() {
		...
		for (var i = 0, l = geometry.attributes.position.count * 3; i < l; i = i + 9) {
			...
			*/
			...
			if (currentEffect == 'effect1') {
				...
				var z = Math.cos((i * thisTime ^ (origTime / time)) / 1000000) * n * (Math.cos(i) * 10) - n2;
			}

			if (currentEffect == 'effect2') {
				var x = Math.cos((i * thisTime ^ 2) / 100000000) * n * (Math.cos(i) * 10) - n2;
				var y = Math.sin((i * thisTime ^ 2) / 100000000 * 2) * n * (Math.cos(i) * 10) - n2;
				var z = Math.sin((i * thisTime ^ 2) / 100000000 * 2) * n * (Math.sin(i) * 10) - n2;
			}

			if (currentEffect == 'effect3') {
				var t = (i * thisTime ^ 2) / 100000000;

				var scale = 2 / (3 - Math.cos(t * 2));

				var x = Math.cos(t) * n * (Math.sin(i) * 10)   //- n2;
				var y = 2 + Math.sin(t * 2) / 2 * n * (Math.sin(i) * 10) //- n2;
				var z = Math.sin( t *    n*(Math.cos(i)*10)  - n2) //- n2;
			...
			if (currentEffect == 'effect6') {
				var x = Math.cos((i * thisTime ^ 2) / 100000000) * n * (Math.cos(i) * 10) - n2;
				var y = Math.sin((i * thisTime ^ 2) / 100000000) * n * (Math.cos(i) * 10) - n2;
				var z = Math.cos((i * thisTime ^ (origTime / time)) / 1000000) * n * (Math.sin(i) * 10) - n2;
			}
		...
		for (var i = 0; i < segments; i++) {
			...
			if (currentEffect == 'effect2') {
				var x = (Math.cos((i * thisTime ^ 2) / 100000000) * n * (Math.cos(i) * 10) - n2) * .9;
				var y = (Math.sin((i * thisTime ^ 2) / 100000000 * 2) * n * (Math.cos(i) * 10) - n2) * .9;
				var z = (Math.sin((i * thisTime ^ 2) / 100000000 * 2) * n * (Math.sin(i) * 10) - n2) * .9;
			}
		...
		//	camera.position.z = camera.position.z - Math.sin((time-origTime)/1000000)*2;
		//console.log(camera.position.z - (time-origTime)/100000);

		time = Date.now() * 0.001;
		//mesh.rotation.y = 3.14159/4;
		//mesh2.rotation.y = 3.14159/4;
		mesh.rotation.z = time * 0.225;
		mesh2.rotation.z = time * 0.225;

		mesh.rotation.x = time * 0.0125 + cursorY / 500;
		mesh2.rotation.x = time * 0.0125 + cursorY / 500;
		mesh.rotation.y = time * 0.0125 + cursorX / 500;
		mesh2.rotation.y = time * 0.0125 + cursorX / 500;
